...
export default class ChatGptViewProvider implements vscode.WebviewViewProvider {
	...
	constructor(private context: vscode.ExtensionContext) {
		this.subscribeToResponse = vscode.workspace.getConfiguration("chatgpt-sql").get("response.showNotification") || false;
		this.autoScroll = !!vscode.workspace.getConfiguration("chatgpt-sql").get("response.autoScroll");
		this.model = vscode.workspace.getConfiguration("chatgpt-sql").get("gpt3.model") as string;
		...
		webviewView.webview.onDidReceiveMessage(async data => {
			switch (data.type) {
				...
				case 'login':
					...
					break;
				case 'openSettings':
					vscode.commands.executeCommand('workbench.action.openSettings', "@ext:gaojian80422.chatgpt-sql-vscode-plugin chatgpt-sql.");

					this.logEvent("settings-opened");
					break;
				case 'openSettingsPrompt':
					vscode.commands.executeCommand('workbench.action.openSettings', "@ext:gaojian80422.chatgpt-sql-vscode-plugin promptPrefix");
	...
	public setProxyServer(): void {
		this.proxyServer = vscode.workspace.getConfiguration("chatgpt-sql").get("proxyServer");
	}

	public setMethod(): void {
		this.loginMethod = vscode.workspace.getConfiguration("chatgpt-sql").get("method") as LoginMethod;
	...
	public setAuthType(): void {
		this.authType = vscode.workspace.getConfiguration("chatgpt-sql").get("authenticationType");
		this.clearSession();
	}
	...
	public setChromeExecutablePath(): void {
		...
		this.chromiumPath = vscode.workspace.getConfiguration("chatgpt-sql").get("chromiumPath") || path;
		this.clearSession();
	}

	public setProfilePath(): void {
		this.profilePath = vscode.workspace.getConfiguration("chatgpt-sql").get("profilePath");
		this.clearSession();
	}
	...
	public async prepareConversation(modelChanged = false): Promise<boolean> {
		...
		const configuration = vscode.workspace.getConfiguration("chatgpt-sql");
				...
				if (!apiKey) {
					vscode.window.showErrorMessage("Please add your API Key to use OpenAI official APIs. Storing the API Key in Settings is discouraged due to security reasons, though you can still opt-in to use it to persist it in settings. Instead you can also temporarily set the API Key one-time: You will need to re-enter after restarting the vs-code.", "Store in session (Recommended)", "Open settings").then(async choice => {
						if (choice === "Open settings") {
							vscode.commands.executeCommand('workbench.action.openSettings', "chatgpt-sql.gpt3.apiKey");
							return false;
	...
	private get systemContext() {
		return vscode.workspace.getConfiguration("chatgpt-sql").get("system") as string;
	}
	...
	public async sendApiRequest(prompt: string, options: { command: string, code?: string, previousAnswer?: string, language?: string; }) {
		...
		this.logEvent("api-request-sent", { "chatgpt-sql.command": options.command, "chatgpt-sql.hasCode": String(!!options.code), "chatgpt-sql.hasPreviousAnswer": String(!!options.previousAnswer) });
		...
		if (outputText !== '') {
			this.sendMessage({ type: 'addResponse', value: outputText, id: this.currentMessageId, autoScroll: this.autoScroll, responseInMarkdown });
			this.sendMessage({ type: 'addResponse', value: outputText, done: true, id: this.currentMessageId, autoScroll: this.autoScroll, responseInMarkdown });
		...
		try {
			...
			this.sendMessage({ type: 'addResponse', value: this.response, done: true, id: this.currentMessageId, autoScroll: this.autoScroll, responseInMarkdown });
			if (this.response !== '') {
				await insertText(question, this.response);
			}
			...
			if (error?.response?.status || error?.response?.statusText) {
				...
				vscode.window.showErrorMessage("An error occured. If this is due to max_token you could try `ChatGPT: Clear Conversation` command and retry sending your prompt.", "Clear conversation and retry").then(async choice => {
					if (choice === "Clear conversation and retry") {
						await vscode.commands.executeCommand("chatgpt-sql-vscode-plugin.clearConversation");
						await delay(250);
...
`;
	...
	private logEvent(eventName: string, properties?: {}): void {
		...
		// this.reporter?.sendTelemetryEvent(eventName, { "chatgpt-sql.loginMethod": this.loginMethod!, "chatgpt-sql.authType": this.authType!, "chatgpt-sql.model": this.model || "unknown", ...properties }, { "chatgpt-sql.questionCounter": this.questionCounter });
		console.debug(eventName, { "chatgpt-sql.loginMethod": this.loginMethod!, "chatgpt-sql.authType": this.authType!, "chatgpt-sql.model": this.model || "unknown", ...properties }, { "chatgpt-sql.questionCounter": this.questionCounter });
	}
	...
	private logError(eventName: string): void {
		...
		// this.reporter?.sendTelemetryErrorEvent(eventName, { "chatgpt-sql.loginMethod": this.loginMethod!, "chatgpt-sql.authType": this.authType!, "chatgpt-sql.model": this.model || "unknown" }, { "chatgpt-sql.questionCounter": this.questionCounter });
		console.error(eventName, { "chatgpt-sql.loginMethod": this.loginMethod!, "chatgpt-sql.authType": this.authType!, "chatgpt-sql.model": this.model || "unknown" }, { "chatgpt-sql.questionCounter": this.questionCounter });
	}
